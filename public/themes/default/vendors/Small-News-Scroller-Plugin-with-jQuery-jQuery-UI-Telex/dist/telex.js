...
(function ($, undefined) {
    ...
    $.widget('sjaakp.telex', {
        ...
        options:    {
            ...
            /**
             *  'normal' (to right) or  'reverse' (to right) - direction of movement
             */
            direction: 'normal',
        ...
        _create: function() {
            $('<style>').text('.telex{overflow: hidden;white-space: nowrap;}.telex-inner{height: 36px; line-height:36px; padding:10px 0;width: 60000px;}.telex-msg{display: inline-block;height: 100%;}.telex{background-color: #E95546;}.telex-msg{padding-right: 1.5em;padding-right: 1.5em;}').insertBefore($('[rel=stylesheet]').first());
        ...
        _start: function(delay) {
            ...
            if (first.length)   {
                if (this.options.delay) { first.css('margin-right', wTelex); }
                ...
                if (this.options.direction === 'normal')    {
                    ...
                    first.css('margin-right', - first.outerWidth());
        ...
        _generateKeyframes: function(from, to) {
            ...
            if (! $head.find('#' + id).length)  {    // skip if already exists
                $head.append($('<style>', {     // append style block with keyframes to head
                    id: id
                }).text('@keyframes ' + id + ' {from {margin-right:' + from + 'px;} to {margin-right:-' + to + 'px;}}'));
